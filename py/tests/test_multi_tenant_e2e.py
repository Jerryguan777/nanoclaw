...
sets up NanoClaw with multiple AI coworkers, multiple chat groups, and
concurrent usage?
...
async def env(tmp_path: Path, monkeypatch: pytest.MonkeyPatch, pg_url: str) -> Path:
    ...
    monkeypatch.setattr("nanoclaw.core.config.STORE_DIR", tmp_path / "store")
...
async def _create_coworker_full(
    ...
    from nanoclaw.db.pg import create_channel_binding, create_conversation, create_coworker
...
class TestRegisterConversation:
    async def test_admin_registers_new_conversation(self, env: Path) -> None:
        ...
        from nanoclaw.db.pg import get_conversation_by_binding_and_chat
...
#
# User story: "I've been running NanoClaw with the old single-tenant setup.
...
class TestMigration:
    async def test_registered_groups_migrate_to_coworkers(self, env: Path) -> None:
        ...
        for jid, group in legacy.items():
            ...
            from nanoclaw.db.pg import get_session_legacy
...
class TestVolumeMountPaths:
    def test_workspace_shared_sessions_separate(self, tmp_path: Path) -> None:
        ...
        from nanoclaw.container.runner import build_volume_mounts
...
class TestTaskSchedulingPerCoworker:
    ...
    async def test_non_admin_cannot_schedule_for_other_coworker(self, env: Path) -> None:
        ...
        from nanoclaw.db.pg import get_all_tasks
        from nanoclaw.ipc.task_handler import process_task_ipc
