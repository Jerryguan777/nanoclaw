"""Integration tests simulating real user flows through NanoClaw.
...
class TestScenarioFirstTimeUser:
    """User just set up NanoClaw, creates tenant, coworker, and conversations."""
...
class TestScenarioMessageFormatting:
    ...
    async def test_messages_formatted_as_xml(self, env: Path) -> None:
        ...
        from nanoclaw.orchestration.router import format_messages
    ...
    async def test_internal_tags_stripped(self, env: Path) -> None:
        ...
        from nanoclaw.orchestration.router import format_outbound
...
class TestScenarioCredentialProxy:
    async def test_proxy_starts_and_serves(self, env: Path) -> None:
        ...
        from nanoclaw.security.credential_proxy import start_credential_proxy
...
class TestScenarioSenderAllowlist:
    async def test_drop_mode_blocks_unauthorized(self, env: Path) -> None:
        from nanoclaw.core.types import NewMessage
        from nanoclaw.security.sender_allowlist import (
            ChatAllowlistEntry,
...
class TestScenarioMountSecurity:
    async def test_blocked_paths_rejected(self, env: Path) -> None:
        from nanoclaw.core.types import AdditionalMount
        from nanoclaw.security.mount_security import validate_mount

        with patch(
            "nanoclaw.security.mount_security.MOUNT_ALLOWLIST_PATH",
            env / "nonexistent.json",
        ):
            from nanoclaw.security.mount_security import reset_cache
