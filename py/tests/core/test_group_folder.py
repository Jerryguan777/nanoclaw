"""Tests for nanoclaw.group_folder."""
...
from nanoclaw.core.group_folder import (
...
def test_resolve_group_folder_path(tmp_path: Path) -> None:
    ...
    groups_dir.mkdir()
    with patch("nanoclaw.core.group_folder.GROUPS_DIR", groups_dir):
...
def test_resolve_group_ipc_path(tmp_path: Path) -> None:
    ...
    data_dir.mkdir()
    with patch("nanoclaw.core.group_folder.DATA_DIR", data_dir):
