...
from nanoclaw.mount_security import (
...
def test_load_mount_allowlist_missing(tmp_path: Path) -> None:
    reset_cache()
    with patch("nanoclaw.mount_security.MOUNT_ALLOWLIST_PATH", tmp_path / "nonexistent.json"):
...
def test_load_mount_allowlist_valid(tmp_path: Path) -> None:
    ...
    with patch("nanoclaw.mount_security.MOUNT_ALLOWLIST_PATH", config_file):
        result = load_mount_allowlist()
...
def test_validate_mount_invalid_container_path() -> None:
    ...
    with patch("nanoclaw.mount_security.load_mount_allowlist") as mock_load:
        from nanoclaw.types import MountAllowlist
